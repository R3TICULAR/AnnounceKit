...
const sharedStyles = css`
  ...
  pre {
    ...
    font-size: 0.8125rem;
    line-height: 1.6;
    background: var(--ak-pre-bg, #f8fafc);
    border: 1px solid var(--ak-border-color, #e2e8f0);
    border-radius: 0.375rem;
    overflow: auto;
    white-space: pre-wrap;
    word-break: break-word;
    color: var(--ak-pre-color, #1e293b);
    min-height: 8rem;
  }

  .empty-msg {
    color: var(--ak-muted-color, #94a3b8);
    font-style: italic;
  }
...
/**
 * Audit report output panel.
 */
@customElement('ak-audit-panel')
export class AkAuditPanel extends LitElement {
  static styles = sharedStyles;

  @property() content = '';
  @property() label = 'Audit Report';
    ...
    return html`
      <div class="panel">
        ...
        <pre aria-label=${this.label}>
          ${this.empty
            ? html`<span class="empty-msg">Run analysis to see the audit report.</span>`
            : this.content}
        </pre>
      </div>
    `;
...
declare global {
  interface HTMLElementTagNameMap { 'ak-audit-panel': AkAuditPanel; }
}
